...
resource "azuread_group_member" "education" {
    ...
    for_each = {for u in azuread_user.adusers: u.mail_nickname => u if u.department == "Education"}
...
resource "azuread_group_member" "managers" {
    ...
    for_each = {for u in azuread_user.adusers: u.mail_nickname => u if u.job_title == "Manager"}
...
resource "azuread_group_member" "engineers" {
    for_each = {for u in azuread_user.adusers: u.mail_nickname => u if u.job_title == "Engineer"}
